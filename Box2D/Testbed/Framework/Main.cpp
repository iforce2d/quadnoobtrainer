...
namespace
{
	...
	GLUI *glui;
    float32 viewZoom = 0.2f;
    b2Vec2 viewCenter(0.0f, 4.5f);
	int tx, ty, tw, th;
	bool rMouseDown;
	b2Vec2 lastp;
}
...
void deviceAttachFunc(struct Gamepad_device * device, void * context) {
    //printf("deviceAttachFunc\n");
    //fflush(stdout);
}

void axisMoveFunc(struct Gamepad_device * device, unsigned int axisID, float value, float lastValue, double timestamp, void * context)
{
    //printf("%d %f\n",axisID,value);
    //fflush(stdout);
    if ( test )
...
int main(int argc, char** argv)
{
    ...
    Gamepad_deviceAttachFunc(deviceAttachFunc, NULL);
    Gamepad_buttonDownFunc(buttonDownFunc, NULL);
    ...
    mainWindow = glutCreateWindow("iforce2d quadcopter trainer");
